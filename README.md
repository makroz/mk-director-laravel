...
| `php artisan mk:make:auth-user {Scope} --with-crud` | Genera CRUD completo del scope + RBAC triada: `AdminController` + `RoleController` + `AbilityController` (SmartController) + 4 FormRequests + 3 JsonResources + 2 DTOs readonly + Repository + Interface + Service + Factory DDD + Seeder con 4 roles predefinidos. ServiceProvider extendido con binding. **Nuevo en v1.6.0-rc4** (MEJORA-02). 18 archivos generados. |
...
| `php artisan mk:auth:create-super-admin --roles=super-admin,admin,editor,viewer` | Siembra los 4 roles predefinidos con abilities específicas (`*`, CRUD completo, view+update, view-only) en una sola corrida. Default BC: solo super-admin. **Nuevo en v1.6.0-rc4** (MEJORA-04). |
