...
- **Top-level `__extraData` response shape (rc12+ opt-in)**: `BaseController::sendResponse()` ahora acepta un 4to parámetro `array $extra` que, cuando se pasa y el flag `mk_director.response.top_level_extra_data` está activo, emite `__extraData` como **sibling top-level de `data`** (forma canónica que matchea `@makroz/core` `MkResponse<T>` y que consume `useMkInfiniteList`). Default `false` en rc12 → `true` en GA. Coexistencia gradual con el shape legacy anidado. Auditoría con `php artisan mk:status --response-shape`.
