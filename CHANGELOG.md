...
- **LAR-01 (CRITICAL SECURITY — IDOR cross-tenant) — `CRUDSmart::update()` y `CRUDSmart::destroy()` ahora firean `beforeQuery` antes de `findOrFail`**. Pre-fix, ambos métodos hacían `$modelClass::findOrFail($id)` estático, bypaseando el hook `beforeQuery` del `MkMultiTenantPlugin` → cualquier tenant autenticado podía escribir/borrar registros de otro tenant (IDOR). `show()` ya tenía el patrón seguro (build query + eager load + fireBeforeQuery + findOrFail); `update()`/`destroy()` ahora lo espejan. **Sin cambio de firma ni de contrato**: BC-safe para consumers. Pinear test `tests/Unit/CRUDSmartTenantIsolationTest.php` (5 tests, source-parsing sobre el trait) para que un refactor futuro no revierta el fix silenciosamente. **Source**: code review 2026-07-03 (`audits/2026-07-03-mk-director-code-review/00-INFORME-CONSOLIDADO.md` § "Los 8 problemas" #1).

- **LAR-03 (HIGH SECURITY — ScopeMismatch escalaba a 500 en lugar de 401) — `MkAuthenticate` ahora catchea `ScopeMismatchException` y la traduce al envelope canónico R-PKG-024 con `__extraData.code = ERR_SCOPE_MISMATCH`**. Pre-fix, el middleware solo catcheaba `AuthenticationException`, pero `ScopeMismatchException` extiende `RuntimeException` (no `AuthenticationException`). Resultado runtime: cuando un request llegaba con un token cuyo `auth_scope` no coincidía con el esperado (otro scope, `no_token`, o `no_scope_ability`), la excepción burbujeaba como 500 genérico de Laravel — bypaseando el envelope canónico, el `__extraData.code`, y rompiendo el contrato explícito del JSDoc ("Mismatches → 401"). Fix: catch específico para `ScopeMismatchException` que invoca `unauthorizedResponse()` con code `ERR_SCOPE_MISMATCH` y `actual_scope` en `__extraData` para que el frontend pueda distinguir "inicie sesión como admin" de un genérico 401. Web routes (no api/*, no JSON) preservan el BC: lanzan `AuthenticationException` para que `Handler::unauthenticated()` redirija a `/login`. Pinear tests `tests/Unit/MkAuthenticateScopeMismatchTest.php` (6 tests: 2 source-parsing + 4 funcionales con Mockery cubriendo los 4 casos: mismatch, `no_scope_ability`, `no_token`, y web BC). **Sin cambio de firma del middleware**: BC-safe. **Source**: code review 2026-07-03 (`audits/2026-07-03-mk-director-code-review/00-INFORME-CONSOLIDADO.md` § "Los 8 problemas" #3).

- **LAR-02 + LAR-08 + XPK-12 (HIGH — auth config merge + `rotate_on_refresh` toggle pineado) — `config/auth_defaults.php` mergeado a `config/mk_director.php` y borrado; los toggles `auth.guards`, `auth.tables.{roles,abilities,role_user,ability_role}`, `auth.ttl.{access_seconds,refresh_seconds}` y `auth.refresh.rotate_on_refresh` ahora viven en el único archivo pineable (`php artisan vendor:publish --tag=mk-config`)**. Pre-fix, `MkServiceProvider::register()` solo cargaba `mk_director.php`; `auth_defaults.php` era código muerto decorativo — los `Config::get('mk_director.auth.tables.abilities', 'abilities')` y `Config::get('mk_director.auth.ttl.access_seconds')` que ya hacen `MkServiceProvider.php`/`TokenIssuer.php`/`Stubs/auth-user.auth-controller.stub` caían al fallback default en runtime, así que pinear `MK_AUTH_TTL_ACCESS_SECONDS=600` en `.env` no funcionaba. Peor: el toggle `auth.refresh.rotate_on_refresh` (rotación B2B por seguridad) NI EXISTÍA en la config canónica del paquete. Fix: mover todas las claves al árbol `auth.*` de `mk_director.php` con env vars explícitos + `filter_var(..., FILTER_VALIDATE_BOOLEAN)` para `rotate_on_refresh` (sin él, `(bool) 'false'` retorna `true` en PHP — footgun). **`auth_defaults.php` borrado** (era código muerto); cualquier consumer que lo haya pineado manualmente pierde la migración silenciosa — el path pineable canónico siempre fue `mk_director.php`. Pinear tests `tests/Unit/AuthConfigMergeTest.php` (12 tests: 7 source-parsing + 3 functional con container fresco + 2 guards de BC). **Sin cambio en runtime para consumers que no pinean config**: los defaults BC (15min access, 7d refresh, no rotate, guards=web, tablas canónicas) matchean el comportamiento previo. **Source**: code review 2026-07-03 (`audits/2026-07-03-mk-director-code-review/00-INFORME-CONSOLIDADO.md` § "Los 8 problemas" #2 + #8 + XPK-12).

- **LAR-04 (HIGH — OpenAPI spec + Swagger UI gateados por config, default OFF) — `GET /mk/openapi.json` y `GET /mk/docs` ahora requieren `MK_OPENAPI_ENABLED=true` (o `'openapi' => ['enabled' => true]` en `config/mk_director.php` publicado) para registrarse; el provider también acepta un array `middleware` opcional que se aplica al `Route::group`**. Pre-fix, `MkServiceProvider::registerOpenApiRoutes()` siempre registraba ambos endpoints para cualquier consumer — fuga de información (schema completo de la API accesible sin auth) + accidental deploy in prod cuando el developer olvidaba comentar el bloque. Fix: gate opt-in vía `(bool) config('mk_director.openapi.enabled', false)` con `filter_var(..., FILTER_VALIDATE_BOOLEAN)` (consistente con el patrón de `rotate_on_refresh`); el array `middleware` (default `[]`) acepta cualquier middleware de Laravel (`mk.auth:{scope}`, `auth.basic`, `web`, etc.) para forzar auth. **BC preservado**: defaults idénticos al comportamiento previo (enabled=false → rutas NO registradas — *mejora de seguridad por default*, antes salían públicas). Los tests existentes de cache key/TTL en `PerformanceImprovementsTest` siguen pasando porque solo dependen de `OpenApiController::CACHE_KEY` + `mk_director.openapi.cache_ttl`, no del routing. Pinear tests `tests/Unit/OpenApiGateTest.php` (4 tests: 3 source-parsing + 1 functional con container mínimo verificando early-return). **Source**: code review 2026-07-03 (`audits/2026-07-03-mk-director-code-review/00-INFORME-CONSOLIDADO.md` § "Los 8 problemas" #4).
...
- **LAR-09 (MEDIUM — `BaseController::sendError()` ahora emite el envelope canónico R-PKG-024 single-level + R-PKG-044 con `__extraData.code`)**. Pre-fix, `sendError($message, $errors = [], $code = 404)` emitía `{success: false, message, errors: {...}, debugMsg}` con `errors` top-level — drift cross-stack con `MkAbility::errorResponse()` y `MkAuthenticate` (que ya emitían `__extraData.code`). El frontend (`@makroz/web`/`@makroz/mobile`) branchéa UX sobre `__extraData.code` para distinguir "credenciales inválidas" de "sesión expirada" de "tenant incorrecto"; con el shape viejo, caía al mensaje genérico y se perdía el affordance. Fix: `sendError()` ahora emite `{success: false, message, data: null, __extraData: {code, errors}, debugMsg}` consistente con el resto del paquete. La firma agrega un 4° parámetro opcional `?string $errorCode = null` para que callers pasen un code explícito; cuando se omite, se deriva del HTTP status vía el helper `defaultErrorCode()` (401 → `ERR_UNAUTHENTICATED`, 403 → `ERR_FORBIDDEN`, 404 → `ERR_NOT_FOUND`, 409 → `ERR_CONFLICT`, 422 → `ERR_VALIDATION`, 429 → `ERR_RATE_LIMITED`, 500 → `ERR_INTERNAL`, default → `ERR_ERROR`). La clave `errors` ahora vive bajo `__extraData.errors` en vez de top-level. **No BC bridge** — RETO regenera Admin desde 0 con v2.0.0 (R-PKG-044), no hay consumers que lean `errors` top-level. Pinear tests `tests/Unit/Controllers/BaseControllerSendErrorEnvelopeTest.php` (5 source-parsing intención) + `tests/Feature/BaseControllerSendErrorEnvelopeE2ETest.php` (6 effectiveness runtime). **Source**: code review 2026-07-03 (`audits/2026-07-03-mk-director-code-review/00-INFORME-CONSOLIDADO.md` § "Los 8 problemas" #9, decisión D4=SÍ).

- **LAR-05 + LAR-11 (MEDIUM — TenantResolver strict real + TenantScope fail-closed opt-in)**. **LAR-05**: pre-fix, `TenantResolver::handle()` leía `strict` con `(bool) $config->get(...)` — PHP footgun: `(bool) 'false' === true`, así que pinear `MK_TENANT_STRICT=false` en `.env` no desactivaba strict (los devs creían estar en modo permisivo cuando seguían en modo bloqueante). Adicionalmente, en strict mode un user autenticado SIN `HasTenantMembership` (sin `getTenantId()`) pasaba el membership check silenciosamente — el código hacía `method_exists ? check : skip`, bypaseando tenant isolation para apps que olvidaron pinear el trait en su User model. Fix: 1) leer `strict` con `filter_var(..., FILTER_VALIDATE_BOOLEAN)` (mismo patrón que `auth.refresh.rotate_on_refresh` y `openapi.enabled`, pineados en F1.2/F1.4); 2) nuevo config `tenant.allowlist_routes` (default: `api/*/auth/login`, `api/*/auth/refresh`, `api/*/auth/forgot`, `api/*/auth/reset`) — rutas en la allowlist exentas del membership check; 3) en strict mode, user sin `getTenantId()` Y ruta NO en allowlist → 403 `ERR_TENANT_MEMBERSHIP_REQUIRED`. Las 3 responses de error (400 missing, 403 mismatch, 403 membership) ahora emiten el envelope canónico R-PKG-024 con `__extraData.code` (vía el helper `canonicalErrorResponse()`), consistente con LAR-09. **LAR-11**: pre-fix, cuando el `TenantContext` era null el scope era no-op (query devolvía TODAS las rows) — conveniente para CLI/queue, PELIGROSO para web porque si el middleware fallaba silenciosamente, el scope filtraba 0 rows y devolvía rows de otros tenants. Fix: nuevo config flag `tenant.fail_closed` (default false, BC-safe) — cuando ON + context null + modelo opt-in, el scope inyecta `where tenant_id = -1` (constante `TenantScope::FAIL_CLOSED_SENTINEL`) → 0 rows. CLI/queue siguen funcionando porque el consumer puede pinear `fail_closed=false` explícito o setear el contexto manualmente. Pinear tests `tests/Unit/Tenancy/TenantStrictFailClosedTest.php` (8 source-parsing intención) + actualización de `TenantResolverTest.php` + `TenantResolverMembershipTest.php` (los asserts viejos sobre `$payload['error']` ahora leen `$payload['__extraData']['code']`). **Source**: code review 2026-07-03 (`audits/2026-07-03-mk-director-code-review/00-INFORME-CONSOLIDADO.md` § "Los 8 problemas" #5 + #11).

- **LAR-06 + LAR-07 (MEDIUM — Magic Cache regex hardening)**. **LAR-06**: pre-fix, el write-verb regex era `/(update|delete|insert(\s+into)?|replace(\s+into)?|upsert|truncate)\s+`?(\w+)`?/i` — `delete` matcheaba `delete()` PHP function calls (cualquier source que mencione `delete(` triggereaba el listener), e `insert(\s+into)?` era opcionalmente permisivo. Fix: el regex ahora REQUIERE tokens SQL específicos después de cada verbo — `delete\s+from`, `insert\s+into`, `replace\s+into` (los tres con INTO/FROM obligatorio), `update` y `truncate` se mantienen como verb-on-their-own (UPDATE table / TRUNCATE table son inequívocos). `upsert` desaparece como top-level verb (Eloquent::upsert() emite `INSERT ... ON DUPLICATE KEY UPDATE`, cubierto por `insert\s+into`). **LAR-07**: pre-fix, el system-tables check usaba `str_contains($query->sql, $table)` — substring match que silenciosamente skipeaba queries legítimas (e.g. `INSERT INTO users (cache_token) VALUES ('x')` porque el SQL contenía `cache`) y skipeaba consumer tables como `cache_stats` por overlap de substring. Fix: reemplazo por token-aware match `/(?:FROM|INTO|UPDATE)\s+`?{table}`?\b/i` — el table name debe aparecer como SQL identifier después de FROM/INTO/UPDATE (o wrapped en backticks). La lista de system tables (`migrations`, `cache`, `cache_locks`, `sessions`, `password_resets`, `password_reset_tokens`, `jobs`, `job_batches`, `failed_jobs`, `telescope_entries`, `telescope_monitoring`) se mantiene. Pinear tests `tests/Unit/MagicCacheRegexHardeningTest.php` (9 nuevos: source-parsing intención por verbo + cobertura per-verb) + actualización de `MkServiceProviderCacheListenerTest.php` y `Console/RegisterGlobalCacheListenerRegexTest.php` (los asserts viejos pineaban el shape legacy `str_contains` y `insert(\s+into)?`). **Source**: code review 2026-07-03 (`audits/2026-07-03-mk-director-code-review/00-INFORME-CONSOLIDADO.md` § "Los 8 problemas" #6 + #7).

- **LAR-12 + LAR-13 + LAR-14 (LOW — defense-in-depth en stub login + ListManager PK correctness + migration stub docs)**. **LAR-12**: pre-fix, el stub login() tenía `$isActiveCheck = Schema::hasColumn(...) && $user->is_active === false` — el null-safe `$user?->getTable() ?? '...'` estaba pineado pero `$user->is_active === false` no, así que si el user lookup devolvía null Y la tabla tenía `is_active`, el `&&` no short-circuiteaba (porque Schema::hasColumn retorna true) y `$user->is_active` reventaba con "Attempt to read property on null". El `if (! $user || ...)` downstream cubría el null para el response path, pero la expresión intermedia crasheaba primero. Fix: `$user?->is_active === false` (null-safe operator) en login(), forgot() y reset(). Mismo rationale que las guards de FK en F1.6. **LAR-13**: pre-fix, `ListManager` usaba el literal `'id'` en 4 sitios (applyColumns cols-allowlist, apply() default orderBy, applySorting() default orderBy, applySorting() allowed merge, sanitizeSortState() allowed). Si el consumer tiene un primary key con nombre custom (`uuid`, `code`, `slug`), el literal `'id'` no matcheaba — el orderBy fallback fallaba / producía SQL incorrecto. Fix: reemplazar el literal con `$model->getKeyName()` en los 4 sitios. Default Eloquent retorna `'id'`, así que BC preservada para consumers con PK estándar. **LAR-14**: pre-fix, el migration stub creaba `{{loginField}}` como `string NOT NULL UNIQUE` pero no documentaba la asimetría entre `--login-field=email` (email es credential, NOT NULL) vs `--login-field!=email` (email puede ser profile field, NULLABLE). Fix: docblock note explicando la semántica + pin en test que `MakeAuthUserCommand::resolveProfileFields()` ya rechazaba colisión `email` como profile field cuando es loginField (regression guard — pineado en `resolveProfileFields()` desde R-PKG-011, ahora test-pineado). Pinear tests `tests/Unit/StubAndListManagerHardeningTest.php` (10 nuevos: null-safe regression guards + ListManager getKeyName() en 4 sitios + migration docblock + email collision regression) + actualización de `Console/MakeAuthUserRPkg027FixesTest.php` (la assertion legacy `$user->getTable()` se ablandó a "null-safe OR legacy" para reflejar el F1.8). **Source**: code review 2026-07-03 (`audits/2026-07-03-mk-director-code-review/00-INFORME-CONSOLIDADO.md` § "Los 8 problemas" #12 + #13 + #14).
