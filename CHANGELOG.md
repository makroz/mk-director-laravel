...
- **9 nuevos audit tests pineando HALLAZGO-NEW-01** en `tests/Unit/Auth/HallazgoNew01PivotTest.php` (archivo nuevo):
  ...
  - Anti-regresión: el listener es no-op si la pivot NO tiene columna `user_type` (BC-safe con cache de `Schema::hasColumn`).
...
- Consumers con pivots MME-polimórficas (`role_user` / `ability_user` con columna `user_type`): `$user->roles()->attach([$id])`, `detach()`, `sync()`, `syncWithoutDetaching()`, `toggle()`, `updateExistingPivot()` ahora setean `user_type = get_class($user)` automáticamente vía las nuevas clases `MkRoleUserPivot` / `MkAbilityUserPivot`. Antes, esos métodos directos NO seteaban `user_type` (solo los helpers `assignRole` / `syncRoles` / `giveAbilityTo` / `syncDirectAbilities` lo hacían via `pivotExtras()`). Workaround aplicado en RETO fase 6 (usar `syncRoles()` en vez de `attach()` directo) ya NO es necesario. Si el consumer quiere opt-out del auto-set, override la relation `roles()` / `directAbilities()` en su modelo sin `->using(MkRoleUserPivot::class)` / `MkAbilityUserPivot::class`.
